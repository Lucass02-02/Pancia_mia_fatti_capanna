...
class FClient
{
    ...
    public static function saveObj(EClient $client): bool { return FEntityManager::getInstance()->saveObject($client); }
    public static function getObj(int $id): ?EClient { return FEntityManager::getInstance()->retriveObject(self::getTable(), $id); }
    public static function getClientByEmail(string $email): ?EClient { return FEntityManager::getInstance()->retriveObjectOnAttribute(self::getTable(), 'email', $email); }
    public static function getClientByNickname(string $nickname): ?EClient { return FEntityManager::getInstance()->retriveObjectOnAttribute(self::getTable(), 'nickname', $nickname); }
    public static function getClientByNameSurname(string $name, string $surname): ?EClient { return FEntityManager::getInstance()->retriveObjectOnTwoAttributes(self::getTable(), 'name', $name, 'surname', $surname); }
    public static function getClientByRememberToken(string $token): ?EClient { $em = FEntityManager::getInstance()->getEntityManager(); return $em->getRepository(EClient::class)->findOneBy(['rememberToken' => $token]);}
    public static function deleteObj(EClient $client): bool { return FEntityManager::getInstance()->deleteObject($client); }
